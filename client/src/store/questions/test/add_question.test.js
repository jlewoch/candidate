import reducer from '../reducer';
import * as actions from '../actions';
import {data, expectedAdd} from './mockData'
const initialState = {
    questions:[],
    requesting: false,
    error: false,
    errorMessage:''
}
describe('add reducer', () => {
  it('should return the initial state', () => {
    expect(reducer(undefined, {})).toEqual(initialState);
  });

  it('should handle ADD_QUESTION_REQUEST', () => {
    const startAction = actions.addQuestionRequest()
    expect(reducer({}, startAction)).toEqual({requesting: true});
  });

  it('should handle ADD_QUESTION_SUCCESS', () => {
    const successAction = {
      type: 'ADD_QUESTION_SUCCESS',
      payload: expectedAdd[4]
      
    };
    expect(reducer({questions: data}, successAction)).toEqual({questions: expectedAdd});
  });

});